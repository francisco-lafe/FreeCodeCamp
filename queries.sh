...
echo "$($PSQL "SELECT name FROM teams FULL JOIN games ON winner_id=team_id OR team_id=opponent_id WHERE round='Eighth-Final' and year='2014' ORDER BY name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT name FROM teams inner JOIN games ON winner_id=team_id ORDER BY name")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT year,name FROM games FULL JOIN teams ON winner_id=team_id WHERE round='Final'")"
